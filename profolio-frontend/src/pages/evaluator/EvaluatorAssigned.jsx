...
import {
  ...
  faTriangleExclamation, faEye, faSave, faArrowLeft,
  faMagnifyingGlass, faFilter, faUser, faChevronDown,
...
const EvaluatorAssigned = () => {
  ...
  const viewPortfolio = async (assignment) => {
    ...
    setEvalForm({ final_score: '', comments: '', recommendations: '', career_readiness: 'developing', verdict: 'passed' })
    try {
  ...
  const filtered = pending.filter(item => {
    ...
    const status = item.portfolios?.status || ''
        ...
        <header className="sticky top-0 z-30 bg-[#060612]/90 backdrop-blur-xl border-b border-white/5 px-6 py-4 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-white font-bold text-lg">
              {selectedPortfolio ? 'Portfolio Review' : 'Assigned Portfolios'}
            </h1>
            <p className="text-gray-500 text-xs">
              {selectedPortfolio
                ? portfolioDetail?.student_profiles?.users?.full_name || 'Loading...'
        ...
        <main className="flex-1 px-6 py-8">
          {loading ? (
            ...
            <div className="flex flex-col gap-6">
              <button
                onClick={() => { setSelectedPortfolio(null); setPortfolioDetail(null); setShowEvalForm(false) }}
                className="flex items-center gap-2 text-gray-400 hover:text-white text-sm transition-colors w-fit"
              >
                <FontAwesomeIcon icon={faArrowLeft} className="text-xs" />
                Back to assigned portfolios
              </button>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  { label: 'Total Pending', value: pending.length, gradient: 'from-amber-500 to-orange-500' },
                  { label: 'AI Reviewed', value: pending.filter(a => a.portfolios?.status === 'ai_reviewed').length, gradient: 'from-violet-500 to-purple-500' },
                  { label: 'Under Review', value: pending.filter(a => a.portfolios?.status === 'under_review').length, gradient: 'from-blue-500 to-cyan-500' },
                ].map((stat, i) => (
              ...
              <div className="border border-white/8 bg-white/[0.03] rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
                  <h2 className="text-white font-bold text-sm">Portfolios for Review</h2>
                  <span className="text-xs text-gray-500">{filtered.length} portfolios</span>
                </div>

                {filtered.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    <FontAwesomeIcon icon={faCircleCheck} className="text-green-400 text-4xl mb-4" />
                    <p className="text-gray-400 text-sm font-medium">
                      {pending.length === 0 ? 'All caught up!' : 'No results match your search.'}
                    </p>
                    <p className="text-gray-600 text-xs mt-1">
                      {pending.length === 0 ? 'No pending evaluations at the moment.' : 'Try adjusting your filters.'}
                    </p>
                  </div>
                ) : (
                  <div className="divide-y divide-white/5">
                    {filtered.map((assignment, i) => {
                      ...
                      const studentName = assignment.portfolios?.student_profiles?.users?.full_name || 'Student'
                      const course = assignment.portfolios?.student_profiles?.course || '—'
                      const school = assignment.portfolios?.student_profiles?.school || '—'
                      const yearLevel = assignment.portfolios?.student_profiles?.year_level || ''
                      ...
                      return (
                        <div key={i} className="flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02] transition-all">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-gray-500 text-xs truncate">{course} · {school}{yearLevel ? ` · ${yearLevel}` : ''}</p>
                          </div>
                          <span className={`hidden sm:inline-flex text-xs font-semibold px-2.5 py-1 rounded-full border ${status.bg} ${status.border} ${status.color} flex-shrink-0`}>
                            {status.label}
                          </span>
                          <button
                            onClick={() => viewPortfolio(assignment)}
...
export default EvaluatorAssigned